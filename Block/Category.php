...
class Category extends Template
{
    ...
    {
		$title = __('Risecommerce Portfolio');  

		if($id = $this->getRequest()->getParam('id')){
			$category = $this->getModel()->load($id);
			$title = $category->getCategoryDiscription();
		}
	...
	public function getModel(){
		return $this->_objectManager->create('Risecommerce\Portfolio\Model\Category');
	}
	
	public function getPortfolios(){
		$portfolios = $this->_objectManager->create('Risecommerce\Portfolio\Model\Portfolio')
			->getCollection()
			->addFieldToFilter('status', 1)->setOrder('order_by','ASC');
		
		if($id = $this->getRequest()->getParam('id')){
			$resourceModel = $this->_objectManager->create('Risecommerce\Portfolio\Model\ResourceModel\Portfolio');
	...
	public function getCategories($portfolio){
		$collection = $this->_objectManager->create('Risecommerce\Portfolio\Model\Stores')
			->getCollection()
			->addFieldToFilter('portfolio_id', $portfolio->getId());
		
		$resourceModel = $this->_objectManager->create('Risecommerce\Portfolio\Model\ResourceModel\Stores');
	...
	public function getCategoriesLink($portfolio){
		...
		if(count($collection)>0){
			$i=0;
			foreach($collection as $item){
				$cate = $this->_objectManager->create('Risecommerce\Portfolio\Model\Category')->getCollection()->addFieldToFilter('category_id', ['eq' => $item->getCategoryId()])->getFirstItem();
				$i++;
